...
export class DriverDetailsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['driverName', 'driverNumber',
   'licenseNumber', 'batchNumber', 'expiryDateLicense', 'expiryDateBatch', 'action'];
  driverForm: DRIVER;
 ...
 addDriverDetails(Drivers: DRIVER): void {
  ...
  this.httpClientService.postDriverData(this.url, Drivers).subscribe(res => {
   ...
   this.formResponseText = 'Driver details has added successfully updated';
     ...
     alert('Seems Somethign wrong!. Engineering are working on it');
 });
...
getData(): void {
   this.httpClientService.getDriverData(this.url).subscribe(res => {
     ...
     this.totalDrivers = this.usersList.length;
   });
